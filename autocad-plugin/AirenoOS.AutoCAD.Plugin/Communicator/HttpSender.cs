...
namespace AirenoOS.AutoCAD.Plugin.Communicator
{
    ...
    internal static class HttpSender
    {
        ...
        /// <summary>
        /// Free-feature highlight: polls /v1/highlight for any pending highlight
        /// requests pushed by the MCP cockpit. Returns the list of {native_ids[]}
        /// that should be flashed in the active drawing. Consume-on-read on the
        /// server side, so the same request never replays on the next poll cycle.
        /// </summary>
        public static async Task<List<HighlightRequest>> FetchHighlightsAsync(Database db)
        {
            ...
            var highlightUrl = System.Text.RegularExpressions.Regex.Replace(
                ...
                System.Text.RegularExpressions.RegexOptions.IgnoreCase);
            var docToken = ProjectTokenManager.GetProjectToken(db);
            var url = $"{highlightUrl}?document_project_token={Uri.EscapeDataString(docToken)}";
